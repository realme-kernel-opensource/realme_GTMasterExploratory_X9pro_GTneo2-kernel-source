...
static u16 firmware_calibrate_status = 0;
...
static int32_t iris_request_firmware(const struct firmware ** fw,
		const uint8_t *name)
{
	int32_t rc = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	IRIS_LOGE("%s: request firmware [OK]: name = %s, size = %zu bytes",
			__func__, name, (*fw)->size);

	return rc;
}

static void iris_release_firmware(const struct firmware ** fw)
{
	if (*fw) {
		release_firmware(*fw);
		*fw = NULL;
...
static int iris_change_lut_type_addr(
...
static int iris_change_dbc_type_addr(void)
{
	int i = 0;
	int rc = -EINVAL;
	u8 ip = IRIS_IP_DBC;
...
static int iris_change_gamma_type_addr(void)
{
	u8 i = 0;
	int rc = -EINVAL;
	u8 ip = IRIS_IP_DPP;
	...
	for (i=0; i<gamma_lut_opt_cnt; i++) {
		...
		if (!lut_popt) {
			IRIS_LOGE("%s(), cann't find valid lut option,"
				" input ip: %#x, opt: %#x.",
...
static int iris_change_sdr2hdr_type_addr(void)
{
	int i = 0;
	int j = 0;
	int rc = -EINVAL;
...
static int iris_change_dither_type_addr(void)
{
	int rc = -EINVAL;
	u8 ip = IRIS_IP_DPP;
	u8 opt_id = 0xF8;
...
static int iris_send_lut_by_dma(void)
{
	int rc = 0;

	/*register level*/
	rc = iris_change_dbc_type_addr();
	rc = iris_change_gamma_type_addr();
	rc = iris_change_sdr2hdr_type_addr();
	rc = iris_change_dither_type_addr();

	return rc;
}

static void iris_parse_panel_nits(const struct firmware *fw)
{
	struct iris_cfg *pcfg = iris_get_cfg();
...
static u8 iris_parse_panel_calibrate_status(const struct firmware *fw)
{
       u8 calibrate_status = 0;
       calibrate_status = fw->data[fw->size-1];
       IRIS_LOGI("%s(), panel calibrate status: 0x%02x.", __func__, calibrate_status);
...
int iris5_parse_lut_cmds(void)
{
	...
	if (!ret) {
		...
		IRIS_LOGI("%s(%d), request name = %s, size: %u.",
			__func__, __LINE__, IRIS_FIRMWARE_NAME, data[0].len);
	} else {
		IRIS_LOGE("%s, can't request %s", __func__, IRIS_FIRMWARE_NAME);
		if (pcfg)
			pcfg->lightup_no_fw = true;
	}
	...
	if (!ret) {
		...
		if ((ccf1_fw->size - ccf1_tail_size) % ccf1_per_pkg_size == 0
			...
			data[1].len = ccf1_fw->size - 3;
			iris_parse_panel_nits(ccf1_fw);
			fw_calibrate_st = iris_parse_panel_calibrate_status(ccf1_fw);
	...
	if (!ret) {
		...
		if ((ccf2_fw->size - ccf2_tail_size) % ccf2_per_pkg_size == 0
			...
			data[2].len = ccf2_fw->size - 1;
			fw_calibrate_st = iris_parse_panel_calibrate_status(ccf2_fw);
	...
	IRIS_LOGI("%s, load firmware %s, state %#x", __func__,
			firmware_loaded == FIRMWARE_LOAD_SUCCESS?"success":"fail", firmware_state);
	if (firmware_state != 0) {
		pcfg = iris_get_cfg();
		ret = iris_attach_cmd_to_ipidx(data, (sizeof(data))/(sizeof(data[0])),
					 pip_index, &pcfg->lut_cmds_cnt);
		if (ret)
			IRIS_LOGE("error for parse ccf and iris5 fw");
...
static void iris_prepare_lut_cmds(struct iris_ip_opt *popt)
{
	int pos = 0;
	...
	IRIS_LOGD("%s(), %p %p len = %d", __func__, &pdesc[pos], popt, popt->len);
	memcpy(&pdesc[pos], popt->cmd,
				sizeof(*pdesc) * popt->len);
	pos += popt->len;
	pcfg->iris_cmds.cmds_index = pos;
}

static void iris_lut_fomat_cmds(u8 lut_type, u8 opt_id)
{
	struct iris_ip_opt *popt = NULL;
	...
	iris_prepare_lut_cmds(popt);
}

int iris_lut_send(
		u8 lut_type, u8 lut_table_index,
		u32 lut_abtable_index)
{
	int i = 0;
	...
	switch (lut_type) {
	case DBC_LUT:
		if (!((lut_table_index <= DBC_HIGH /*&& lut_table_index >= DBC_INIT*/) || //CID799210
		...
		for (i = 0; i < len; i++) {
			...
			iris_lut_fomat_cmds(lut_type, lut_opt_id);
		}
		IRIS_LOGI("%s: call DBC_LUT, index: %#x.", __func__, lut_table_index);
	break;

	case CM_LUT:
		if (lut_table_index >= cm_lut_opt_cnt) { //CID89930
	...
	case SDR2HDR_LUT:
		if (lut_table_index > (SDR2HDR_LEVEL5 | (SDR2HDR_INV_UV1<<4))) { //CID89930
			IRIS_LOGW("%s(%d), invalid table index %d of type: %#x.",
				__func__, __LINE__, lut_table_index, lut_type);
			break;
		}
	...
	case SCALER1D_PP_LUT:
		if (lut_table_index >= SCALER1D_LUT_NUMBER) { //CID89930
			IRIS_LOGW("%s(%d), invalid table index %d of type: %#x.",
				__func__, __LINE__, lut_table_index, lut_type);
			break;
		}
	...
	case GAMMA_LUT:
		if (lut_table_index >= gamma_lut_opt_cnt) { //CID89930
			IRIS_LOGW("%s(%d), invalid table index %d of type: %#x.",
				__func__, __LINE__, lut_table_index, lut_type);
			break;
		}
	...
	case FRC_PHASE_LUT:
		if (lut_table_index >= FRC_PHASE_TYPE_CNT) { //CID89930
			IRIS_LOGW("%s(%d), invalid table index %d of type: %#x.",
				__func__, __LINE__, lut_table_index, lut_type);
			break;
		}
	...
	IRIS_LOGD("%s: called lut_type = %#x, lut_table_index = %#x",
		__func__, lut_type, lut_table_index);
	IRIS_LOGD("%s: lut_abtable_index = %#x, cmd_cnt = %d, max_cnt = %d",
		__func__, lut_abtable_index, pcfg->iris_cmds.cmds_index,
			...
			if (lutpos == 0xFFE00000) {
				...
				if (index_i % 2 == 0) {
					if (index_i % 4 == 0) /* EVEN*/
						...
						lut_fw_index = index_i / 4 + LUT_LEN / 4;
...
void iris_maxcll_lut_update(enum LUT_TYPE lutType, u32 lutpos)
{
			...
			if (lutpos == 0xFFFF0000) {
			  ...
			  LUTUVY_fw[lut_fw_index + LUT_LEN + LUT_LEN + LUT_LEN + LUT_LEN + LUT_LEN] = lut_luty[index_i];
				...
				if (index_i % 2 == 0)
				{
					...
					LUTUVY_fw[lut_fw_index + LUT_LEN / 2 + LUT_LEN / 2] = lut_lutuv[index_i];
					LUTUVY_fw[lut_fw_index + LUT_LEN / 2 + LUT_LEN / 2 + LUT_LEN / 2] = lut_lutuv[index_i];
					LUTUVY_fw[lut_fw_index + LUT_LEN / 2 + LUT_LEN / 2 + LUT_LEN / 2 + LUT_LEN / 2] = lut_luty[index_i];
					LUTUVY_fw[lut_fw_index + LUT_LEN / 2 + LUT_LEN / 2 + LUT_LEN / 2 + LUT_LEN / 2 + LUT_LEN / 2] = lut_luty[index_i];
				}
			}
		}
	} else {
	...
	for (lut_block_index = 0; lut_block_index < lut_block_cnt; lut_block_index++){
		...
		if (lutpos == 0xFFFF0000)
			...
			ocp_dbus_header.address = dbus_addr_start + lut_block_index*SDR2HDR_LUT_BLOCK_ADDRESS_INC + lutpos * SDR2HDR_LUT_BLOCK_SIZE/2;
