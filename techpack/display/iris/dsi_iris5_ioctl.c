...
int iris5_operate_tool(struct msm_iris_operate_value *argp)
{
	...
	if (pcfg == NULL || pcfg->valid < 2) {
		IRIS_LOGD("Target display does not exist!");
		return -EPERM;
	}
...
int iris_configure(u32 display, u32 type, u32 value)
{
	...
	IRIS_LOGW("%s type=0x%04x, value=%d", __func__, type, value);
	...
	*/
	...
	case IRIS_LCE_LEVEL:
		if (pqlt_cur_setting->pq_setting.lcelevel != (value & 0x7)) {
			...
			if (pqlt_cur_setting->pq_setting.lcelevel)
				iris_cm_csc_cmp_set(true);
			else
				iris_cm_csc_cmp_set(false);
		}
		break;
	...
	case IRIS_CHIP_VERSION:
		...
		break;
	case IRIS_CM_PARA_SET:
		pqlt_cur_setting->cm_para_level = value;
		if (pqlt_cur_setting->cm_para_level > 1) {
			goto error;
		}
		iris_cm_para_level_set(pqlt_cur_setting->cm_para_level);
		break;
	default:
		goto error;
		break;
...
int iris_configure_get(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DBG_TARGET_REGADDR_VALUE_GET:
		IRIS_LOGE("%s:%d, pcfg->abypss_ctrl.abypass_mode = %d", __func__, __LINE__, pcfg->abypss_ctrl.abypass_mode);
	...
	case IRIS_MEMC_OSD:
		*values = pcfg1->frc_setting.memc_osd;
		break;
	case IRIS_CM_PARA_SET:
		*values = pqlt_cur_setting->cm_para_level;;
		break;
	default:
		return -EFAULT;
	}
