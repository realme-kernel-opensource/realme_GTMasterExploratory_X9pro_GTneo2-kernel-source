...
int max77932_hardware_config(void)
{
    ...
    charge_pump_config_interface(CHARGE_PUMP_ADDRESS_REG06, (BIT(4) | BIT(5)), (BIT(4) | BIT(5)));
...
static int charge_pump_get_hwid(struct chip_charge_pump *chip)
{
    ...
    if (chip->charge_pump_hwid_gpio <= 0) {
        ...
        ret = read_value_from_reg(CHARGE_PUMP_ADDRESS_REG31);
        if(ret == HWID_DA9313) {
    ...
    if (gpio_state_pull_up == 0 && gpio_state_pull_down == 0) {
        ...
        ret = read_value_from_reg(CHARGE_PUMP_ADDRESS_REG16);
