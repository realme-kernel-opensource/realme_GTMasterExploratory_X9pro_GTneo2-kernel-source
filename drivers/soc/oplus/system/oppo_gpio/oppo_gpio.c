...
static int dual_sim_det_uim2_to_real_sim()
{
	...
	if (oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio >= 0) {
		esim_status = oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio_status;

		if (esim_status == 1) {
			OPLUS_GPIO_MSG("esim is up, switch to real sim\n");
			uim_qmi_power_down_req(2);
			msleep(200);
			gpio_direction_output(oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio, 0);
			oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio_status = 0;
		}
	}
...
static void dual_sim_det_uim2_to_esim(struct work_struct *work)
{
	if (oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio >= 0) {
		...
		gpio_direction_output(oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio, 1);
		oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio_status = 1;
...
static int dual_sim_det_init(struct platform_device *pdev, void *gpio_info_ptr)
{
	...
	struct oppo_gpio_info *gpio_info = (struct oppo_gpio_info *)gpio_info_ptr;
...
static void init_esim_status()
{
	if (strstr(saved_command_line, "esim.status=1")) {
		oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio_status = 1;
		oppo_gpio_info_table[GPIO_TYPE_ESIM_PRESENT].gpio_status = 0;
	} else {
		oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio_status = 0;
		oppo_gpio_info_table[GPIO_TYPE_ESIM_PRESENT].gpio_status = 1;
	}
	OPLUS_GPIO_MSG(" %d", oppo_gpio_info_table[GPIO_TYPE_ESIM].gpio_status);
}
...
static int oppo_gpio_probe(struct platform_device *pdev)
{
	...
	if (!valid_gpio) {
		OPLUS_GPIO_MSG("no valid oppo gpio\n");
		goto err_no_gpio;
	}
